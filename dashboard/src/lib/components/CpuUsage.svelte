<script lang="ts">
  ...
  function ensureSkeleton(width: number, height: number) {
    ...
    root.append('path').attr('class', 'line-user').attr('fill', 'none')
      .attr('stroke', COLOR_USER).attr('stroke-width', '1.5')
    root.append('path').attr('class', 'line-sys').attr('fill', 'none')
      .attr('stroke', COLOR_SYS).attr('stroke-width', '1.5')
    ...
    legend.append('text').attr('x', 8).attr('dy', '0.35em')
      .attr('fill', 'rgba(200,210,240,0.7)')
      .attr('font-family', '"JetBrains Mono","Fira Mono",monospace')
      .attr('font-size', '10px').text('user')
    legend.append('circle').attr('cx', 44).attr('r', 4).attr('fill', COLOR_SYS)
    legend.append('text').attr('x', 52).attr('dy', '0.35em')
      .attr('fill', 'rgba(200,210,240,0.7)')
      .attr('font-family', '"JetBrains Mono","Fira Mono",monospace')
      .attr('font-size', '10px').text('sys')
    // Crosshair + overlay
    root.append('line').attr('class', 'crosshair')
  ...
  function draw(pts: { t: number; user: number; sys: number }[]) {
    ...
    const lineUser = d3.line<{ t: number; user: number; sys: number }>()
      .x((d) => xScale(d.t)).y((d) => yScale(d.user)).curve(d3.curveMonotoneX)
    const lineSys = d3.line<{ t: number; user: number; sys: number }>()
      .x((d) => xScale(d.t)).y((d) => yScale(d.sys)).curve(d3.curveMonotoneX)
    ...
    root.select<SVGRectElement>('rect.overlay')
      ...
      .on('mousemove', function (event: MouseEvent) {
        ...
        const pt = pts[idx]
        crosshair.attr('x1', xScale(pt.t)).attr('x2', xScale(pt.t)).attr('y1', 0).attr('y2', h).attr('opacity', '1')
